...
class AttitudeEstimationAz:
    dt = 0.01
    dim_x = 2
    dim_z = 1

    Q = np.eye(dim_x) * 0.1 * dt
    R = np.eye(dim_z) * 1000

    F = np.eye(dim_x)

    # H = np.array([[-1, -1]]) * VehicleParams.g
    ...
    def update(self, z: np.array):
        ...
        y = z - self.h(x)
        self.H = np.array([[-np.sin(x[0]) * np.cos(x[1]), -np.cos(x[0]) * np.sin(x[1])]]) * VehicleParams.g
...
if __name__ == '__main__':
    ...
    estimator = AttitudeEstimationAz()
    u = np.array([0.01, 0.1])
    z = np.array([-9.81])
