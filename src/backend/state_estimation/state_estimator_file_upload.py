...
def upload_estimated_states(tab_name: str, data: pd.DataFrame, columns: list[str], cols: st.columns) -> pd.DataFrame:
    ...
    if uploaded_file is not None:
        if cols[1].button("Update the state estimation data", key=f"{tab_name} save estimation data button"):
            ...
            data.loc[state_estimation_df.index[0]:state_estimation_df.index[-1],
            state_estimation_df.columns] = state_estimation_df.values
    return data
